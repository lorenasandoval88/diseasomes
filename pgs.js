...
pgs.loadScore = async (entry = 'PGS000004', build = 37, range) => {
    ...
    if (range) {
        ...
        txt = pgs.pako.inflate(await (await fetch(url)).arrayBuffer(), {
            to: 'string'
        })
    }
    return txt
...
pgs.textArea = async (entry = 'PGS000004', build = 37, range = 20000) => {
    ...
    let response = await fetch(`https://ftp.ebi.ac.uk/pub/databases/spot/pgs/scores/${entry}/ScoringFiles/Harmonized/${entry}_hmPOS_GRCh${build}.txt.gz`, {
        method: 'HEAD'
    })
...
pgs.ini = () => { // act on context, such as search parameters. Not called automatically here.
    ...
    if (location.search.length > 3) {
        location.search.slice(1).split('&').map(x => {
            aa = x.split('=');
            pgs.parms[aa[0]] = aa[1]
        })
    }
...
if (typeof (define) != "undefined") {
    ...
    define(['https://cdnjs.cloudflare.com/ajax/libs/plotly.js/1.33.1/plotly.min.js',
        ...
        'https://cdnjs.cloudflare.com/ajax/libs/localforage/1.9.0/localforage.min.js'
    ], function (Plotly, pako, localforage) {
        pgs.Plotly = Plotly
        pgs.pako = pako
        pgs.localforage = localforage
        return pgs
